...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=10, ratio=1):
        ...
        layer2_planes = int(128 * ratio)
        self.layer2 = self._make_layer(block, layer2_planes, layers[1], stride=2)

        layer3_planes = int(256 * ratio)
        self.layer3 = self._make_layer(block, layer3_planes, layers[2], stride=2)

        layer4_planes = int(512 * ratio)
        self.layer4 = self._make_layer(block, layer4_planes, layers[3], stride=2)

        self.fc = nn.Linear(layer4_planes * block.expansion, num_classes)
